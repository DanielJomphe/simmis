...
// Exact domain or a subdomain of it — NOT a loose substring (so "linkedin.com"
// never matches "foo-linkedin.com.evil.com").
function allowlisted(host, allowlist) {
...
async function capturePage(tab) {
  ...
  // The sensitive denylist applies to MANUAL capture too — a stray shortcut on
  // a bank/webmail/auth page must never capture it.
  if (isSensitive(hostname(tab.url))) {
    console.warn("[simmis] refusing to capture a sensitive page");
    return;
  }
  try {
...
async function shouldAutoCapture(url) {
  ...
  if (!allowlisted(host, allowlist)) return false;
  const now = Date.now();
